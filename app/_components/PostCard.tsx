...
export default function PostCard({ post, user }: { post: PostFromAPI; user: { id: string; name?: string; image?: string } }) {
  return (
    <article className="bg-white/70 backdrop-blur-sm rounded-2xl mb-5 shadow-lg border border-white/20 overflow-hidden">
      ...
      <PostActions postId={post.id} userId={user.id} initialLikes={post.likes} initialComments={post.comments} />
    </article>
  );
}
