...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!userId || !content) return NextResponse.json({ error: "User ID and content required" }, { status: 400 });
    ...
    return NextResponse.json({ success: true, comment: { ...comment, createdAt: comment.createdAt.toISOString() } });
  } catch (err) {
    console.error(err);
