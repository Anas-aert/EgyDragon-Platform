...
const Profile = async ({ searchParams }: { searchParams?: { id?: string } }) => {
  const userId = searchParams?.id;
  if (!userId) return <p>User ID not provided</p>;
  ...
  const res = await fetch(`https://egydragon-anas.vercel.app/api/userPosts/${userId}`, {
    cache: "no-store",
    headers: cookieHeader.length ? { Cookie: cookieHeader } : undefined,
  });
      ...
      <div className="container mx-auto px-4 max-w-6xl flex flex-col gap-8">
        {session ? (
          <div className="bg-white shadow-lg rounded-2xl p-8 w-full max-w-md text-center border border-gray-100 mx-auto">
            ...
            {session.user?.image && (
              <Image width={128} height={128} src={session.user.image} alt="User" className="w-32 h-32 rounded-full mx-auto mb-4" />
            )}
